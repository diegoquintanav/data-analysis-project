...
```{r}
df$X6.Orientation <- factor(df$X6.Orientation, levels=2:5, labels = c('North','East', 'South','West'))
df$X8.GlazingAreaDistribution <- factor(df$X8.GlazingAreaDistribution, levels = 0:5, labels = c("No Glazing", "Uniform", 'North','East', 'South','West'))
```
...
```{r}
df.continous <- select(df, -grep("^X[6,8]", colnames(df)))
```

## Separating datasets in inputs and response variables

```{r}
df.X <- df[,-grep("Load$", colnames(df.continous))]
df.y <- df[,grep("Load$", colnames(df.continous))]
```
...
mdRobustDistances <- mahalanobis2(df.continous, robust = TRUE, inv.pseudo = TRUE, plot = TRUE)
```

```{r}
mdRobustDistances <- mahalanobis2(df.continous, robust = TRUE, inv.pseudo = FALSE, plot = TRUE)
```
...
chart.Correlation(df.continous, histogram=TRUE)
```
...
```{r eval=TRUE}
hldata <- select(df, -(y2.CoolingLoad))
cldata <- select(df, -(y1.HeatingLoad))
...
```{r}
df.2 <- select(df.continous, -(X4.RoofArea))
det(cov(df.2))
# [1] 146.1685
```
...
```{r}
Moutlier(scale(df.2, center = TRUE, scale = colSums(df.2)),
         quantile = 0.999,
         plot = TRUE)
```
...
```{r}
set.seed(42)
mdClassicDistances2 <- mahalanobis2(df.2, robust = FALSE, inv.pseudo = FALSE, plot = TRUE, tol = 0.999)
mdRobustDistances2 <- mahalanobis2(df.2, robust = TRUE, inv.pseudo = FALSE, plot = TRUE, tol = 0.999)
```
```{r}
crd <- data.frame(rn = rownames(df.2), dc = mdClassicDistances2$distances, dr = mdRobustDistances2$distances)
...
```{r}
targets <- grep("Load$", colnames(df))
categoricals <- grep("^X[6,8]", colnames(df))
PCA(df, quali.sup = categoricals, quanti.sup = targets, scale.unit = TRUE)
```
